...
class HomeScreen extends StatelessWidget {
  ...
  // --- LOGIC BARU: Hitung Total Saldo dari Semua Rekening ---
  int _calculateTotalAccountBalance(List<AccountModel> accounts) {
    double total = 0.0;
    for (var account in accounts) {
      String cleanString = account.saldo
          .replaceAll(RegExp(r'[Rp\.]'), '')
          .replaceAll(',', '') 
          .trim();
          
      double amount = double.tryParse(cleanString) ?? 0.0;
      total += amount;
    }
    return total.toInt();
  }

  // --- LOGIC LAMA: Hitung Data Mingguan (Senin-Minggu) ---
  ...
  Widget build(BuildContext context) {
    ...
    final int totalRealBalance = _calculateTotalAccountBalance(accountProvider.accounts);
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _buildFinanceCards(totalRealBalance, transactionProvider),
                  const SizedBox(height: 24),
                  ...
                  _buildTransactionHistorySection(context, transactionProvider),
                  const SizedBox(height: 24),
  ...
  Widget _buildHeader(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween, // Pastikan layout tetap rapi
        children: [
          ...
          // Bagian Kanan: Lonceng dihapus, diganti dengan SizedBox kosong atau widget lain jika perlu
          // const SizedBox(width: 40), // Optional: Jika ingin menjaga spacing
        ],
      ),
    );
  }
  ...
  Widget _buildFinanceCards(int totalBalance, TransactionProvider transactionProvider) {
    return Row(
      children: [
        Expanded(
          child: _buildCardItem(
            title: "Total Saldo",
            amount: TransactionProvider.formatRupiah(totalBalance),
        ...
        Expanded(
          child: _buildCardItem(
            title: "Pengeluaran",
            amount: TransactionProvider.formatRupiah(transactionProvider.totalExpense),
  ...
  // 4. RIWAYAT TRANSAKSI
  Widget _buildTransactionHistorySection(BuildContext context, TransactionProvider provider) {
    final reversedTransactions = provider.transactions.reversed.toList();
            ...
            : SizedBox(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    final item = reversedTransactions[index];
                    return Row(
                      children: [
